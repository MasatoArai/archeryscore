...
ScoreCTRL.prototype.init = function (confObj, stflag) {
    ...
    $("#score_pad_wrapper a").each(function (i) {
        ...
        var lowestScore = me.gameconfig.targettype.score[0];
            if (((lowestScore > val) && (val != -1))||(!me.gameconfig.enableX&&val == 11)){ $(this).addClass("button_disable");
                                                   }
        $(this).on('touchstart mousedown', function (e) {
            e.preventDefault();
            if (me.contents.lock[me.status.nowRound][me.status.nowEnd]) return false;
            if (((lowestScore > val) && (val != -1))||(!me.gameconfig.enableX&&val == 11)){
                return false;
            }
...
ScoreCTRL.prototype.setTargetFace = function () {
    ...
    this.targetFace.info.infotext = this.targetFace.info.text("Score number is locked").move("50%", "96%").fill("#fff");
    ...
    targetparam.face = this.gameconfig.targettype.score;
    targetparam.maxr = 90;
    targetparam.step = targetparam.maxr/(targetparam.face.length-1);
    targetparam.color = "target";    
    
    var isDisabledX = false;
    var isInnerTen = false;
    if(this.gameconfig.gametype.canSelectX==true&&this.gameconfig.enableX==false){
        isDisabledX = true;
        if((this.gameconfig.gametype.value=="FITA18"||this.gameconfig.gametype.value=="vegas")&&(this.gameconfig.bowtype == "CompoundBow")){
            isInnerTen = true;
        }
    }
    var xr = 0;
    for (var i = 0; i < targetparam.face.length; i++) {
            if (targetparam.face[i] == 11) {
                if(isDisabledX)break;
                xr = targetparam.step / 2;
            }

            if (isInnerTen && (targetparam.face[i] == 10)) {
                    xr -= targetparam.step / 2;
            }
...
ScoreCTRL.prototype.addHitPointInit = function () {
    ...
    $(me.targetFace.box.node).on({
        ...
        'touchend mouseup': function (e) {
            ...
            me.targetFace.isReset = false;
            me.targetFace.mousemodeGap = {left:0,top:0};
    ...
    function directHit(e, tstart) {
        ...
        if (isTouch && me.targetFace.isReset && tstart) {//mousemode
            var arrowPos = $(me.targetFace.hit[me.targetFace.selecthit].node).offset();
            var arrowr = parCtoPx(parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("rx")))
            me.targetFace.mousemodeGap.left=arrowPos.left-touchX+arrowr;
            me.targetFace.mousemodeGap.top=arrowPos.top-touchY+arrowr;
        }
        
            touchX+=me.targetFace.mousemodeGap.left;
            touchY+=me.targetFace.mousemodeGap.top;

        if (storageCTRL.isUseGap() && isTouch && me.targetFace.mousemodeGap.top==0) {//フィンガーギャップの指定
            touchY = cal(touchY,cal(cal(60,storageCTRL.getGapRange(),"*"),100,"/"),"-");
            //touchY -= 60 * storageCTRL.getGapRange() / 100;
        }

        var center = {};//ターゲットセンターのピクセルを取得
        ...
        */
        ...
        if (isTouch && isReset && tstart) {//mousemode
            ...
            me.status.mousemode.y = touchYonFacePx;            me.status.mousemode.touchX=parCtoPx(parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cx")));      me.status.mousemode.touchY=parCtoPx(parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cy")));
...
//console.debug(touchXonFacePx);
    ...
    function getScore() {
        ...
        assR *= 100;
        var assL = parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cx")) - 50;//50は中心からの長さを図るため50%の意
        assL = Math.abs(assL);
        assL = Math.floor(assL *= 100);

        var assT = parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cy")) - 50;
        assT = Math.abs(assT);
    ...
    function setHitLog() {
        ...
        tararr.x = parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cx"));
        tararr.y = parseFloat(me.targetFace.hit[me.targetFace.selecthit].attr("cy"));
